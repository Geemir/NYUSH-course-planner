...
export interface MigrationRehearsalResult { ok: boolean; migrationCount: number; userCount: number; sessionCount: number; planCount: number; revision: number; snapshotVersion: number; correctionTablesPresent: boolean }
...
export async function runMigrationRehearsal(): Promise<MigrationRehearsalResult> {
  ...
  const [userRows, sessionRows, planRows, journal, correctionTables, plan] = await Promise.all([
