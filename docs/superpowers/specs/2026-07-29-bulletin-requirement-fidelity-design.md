...
This design covers NYU Shanghai Core, major, and minor requirement pages, their normalized catalog records, publication validation, the Degree Progress requirement UI, fallback data, and production rollout to Neon and Vercel.

It does not change New York study-away course discovery, the plan format, user authentication, advisor policy, or the correction workflow. It does not attempt to infer undocumented substitutions or advisor exceptions.
...
1. Ship readers and UI that understand both the current program schema and `schemaVersion: 2` source-faithful documents. Old records continue to use the current view temporarily.
...
8. Run production API and browser smoke checks for Data Science, Core IPC, one minor, one concentration-heavy major, and mobile Progress.
